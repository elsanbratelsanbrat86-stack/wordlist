...
In order to use SCOWL the database must first be created To do so simply type:

    make

which will create the sqlite3 file `scowl.db` which is all that you need for
most operations.  If using a release distribution the database is created
directly from `scowl.txt`, otherwise it is created from the source files in
the `data/` directly and `scowl.txt` can be created with `make scowl.txt`.
...
To modify SCOWL it is necessary to checkout a copy from git as the release
distribution does not include the required source files.
